...
const AllCourses = () => {
  ...
  const getAllCoursesUser = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        // Initialize showModal array to false for each course
  ...
  const handleShow = (courseIndex, coursePrice, courseId, courseTitle) => {
    if (coursePrice.toLowerCase() === 'free') {
  ...
  // Input change handler for card details
  const handleChange = (e) => {
    setCardDetails({ ...cardDetails, [e.target.name]: e.target.value });
  };

  // Check if course is paid (price contains a digit)
